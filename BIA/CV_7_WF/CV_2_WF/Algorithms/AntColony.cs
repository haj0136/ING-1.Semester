    ...
    public class AntColony : IAlgorithm
    {
        ...
        public float[,] StartAlgorithm(AbstractFunction testFunction, int iterations, ILNumerics.Drawing.ILGroup plotCube, Panel panel1, List<ILNumerics.Drawing.ILPoints> listOfPoints, TextBox outPutTextBox)
        {
            ...
            for (int i = 0; i < iterations; i++)
            {
                ...
                {
                    population.Ants[k].PheromoneTable = new Dictionary<(City, City), float>(pheromoneTable);
                    for (int l = 0; l < pathLenght - 1; l++)
                    {
                        ...
                        UpdatePheromoneTable(population.Ants[k]);
                    }
                }
                ...
                if (population.BestAnt.CostValue < bestResult.CostValue)
                {
                    ...
                    pheromoneTable = new Dictionary<(City, City), float>(population.BestAnt.PheromoneTable);
                }
        ...
        private void UpdatePheromoneTable(Ant ant)
        {
            for (int i = 0; i < ant.PheromoneTable.Count; i++)
            {
                var edge = ant.PheromoneTable.ElementAt(i);
                ...
                ant.PheromoneTable.ElementAt(i);
            }
        }
